...
import { fleet_information_label, fleet_name_label, fleet_numb_label, State_label, Tsp_label } from "../../../components/labels and messges";

export default PackageListForDriver = (props) => {
                            ...
                            <View style={styles.flexWrap}>
                                {
                                    newList.map((nlist, i) => (
                                        <View key={i} style={{ width: '50%', padding: 10 }}>
                                            ...
                                            image={{ uri: `${API_URL}/api/v0.1/files?filename=${nlist.packageEntry.imgs[0].Key}&random=${Math.random().toString(36).substring(7)}`, headers: { Authorization: `Bearer ${userToken}` } }}
                            ...
                            <View style={styles.flexWrap}>
                                {
                                    packagelist.map((pklist, i) => (
                                        <View key={i} style={{ width: '50%', padding: 10 }}>
                                            ...
                                            image={{ uri: `${API_URL}/api/v0.1/files?filename=${pklist.packageEntry.imgs[0].Key}&random=${Math.random().toString(36).substring(7)}`, headers: { Authorization: `Bearer ${userToken}` } }}
                                ...
                                delilist.map((delilist, i) => (
                                    <View key={i} style={{ width: '50%', padding: 10 }}>
                                        ...
                                        <PgList onPress={() => toDetail(delilist._id)} 
                                         image={{ uri: `${API_URL}/api/v0.1/files?filename=${delilist.packageEntry.imgs[0].Key}&random=${Math.random().toString(36).substring(7)}`, headers: { Authorization: `Bearer ${userToken}` } }}
            ...
            <View style={{ flex: 0.3,flexDirection: 'row',padding: 10 }}>
                ...
                <View style={{ flex: 1,paddingRight: 5}}>
                    <STComponent
                        ...
                        title={choiceData.city? choiceData.city:State_label}
                        paddingTop = {0}
                        style={{height: 40}}
                        height={40}
            ...
            <View style={{ flex: 5, flexDirection: 'column', justifyContent: 'center', padding: 2, paddingTop: 2 }}>
                ...
                >
                    <View style={{ flex: 5,justifyContent: 'center' }}>
                        <ScrollView 
                            refreshControl={
                                <RefreshControl refreshing={loading} onRefresh={()=>newPackages()} />
                            }
                        >
                            {
                                page == 0 ?
